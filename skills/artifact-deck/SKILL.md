---
name: artifact-deck
description: 발표자료(덱·슬라이드)를 claude.ai Artifact로 만든다. 새 덱뿐 아니라 기존 가이드·문서·노트·README를 발표자료로 옮기는 요청에도 쓴다. "발표자료 만들어줘", "슬라이드 만들어", "덱 작성", "프레젠테이션 만들어", "이 문서를 발표자료로", "아티팩트로 발표자료", "make a deck", "create slides", "turn this doc into a presentation" 같은 요청에서 로드한다. 슬라이드 안에서 조작(입력·슬라이더·계산)이나 스크립트가 필요하거나, 발표자 노트와 청중 창 동기화가 필요하거나, 형식을 따로 정하지 않았으면 이 스킬로 만든다. PPTX·PDF로 내려받거나 슬라이드 편집기에서 고칠 덱이면 Artifact의 Slides 타입을 쓰고, .pptx 파일을 요구하면 쓰지 않는다. 발표 요령·대본 연습처럼 덱을 만들지 않는 질문에는 쓰지 않는다.
---

# Artifact 발표 덱

발표자료는 처음부터 쓰지 않고 **이 스킬의 템플릿에서 시작한다.** 템플릿을 복사해 `<slide>` 태그로 슬라이드 내용만 쓰면 되고, 넘기기·전체화면·슬라이드 목록·발표자 노트·창 동기화는 틀 스크립트(jsDelivr의 `@s-dante/artifact-deck`)가 맡는다.

## 0. Slides 타입과 가르기

| 요구 | 쓰는 것 |
| --- | --- |
| 슬라이드 안의 조작(입력·슬라이더·계산), 페이지 스크립트 | 이 스킬 |
| 발표자 노트, 발표자·청중 창 동기화 | 이 스킬 |
| PPTX·PDF 다운로드, 슬라이드 편집기로 수정·코멘트 | Artifact의 Slides 타입 |
| `.pptx` 파일 자체 | 쓰지 않는다 (pptx 스킬 등) |

사용자가 형식을 정하지 않았으면 이 스킬로 만들고, 끝에 "PPTX가 필요하면 Slides 타입으로 옮길 수 있다"고 한 줄 알린다. 기존 문서를 덱으로 옮길 때는 문서를 먼저 읽고 슬라이드별 한 줄 메시지로 목차를 뽑는다.
...
3. `<template id="slides">` 안의 `<slide>`를 내용으로 채운다. 문법은 템플릿 머리 주석을 따른다.
   ...
   - `label`은 슬라이드 목록에 보이는 이름, `section`은 그 슬라이드에서 시작하는 섹션 이름이다.
...
- **틀 부분은 고치지 않는다**: 셸 마크업(`.shell` 이하)과 틀 CSS(`body`, `.shell`, `.bar`, `.stage`, `.viewport`, `.canvas`, `.panel`, `.notes`, `.nav`, `.strip` 등)는 `deck.js`와 짝이다. 셸은 화면 높이에 맞춰 페이지 스크롤 없이 배치되고, 슬라이드 목록은 가로·세로·전체 펼치기(바둑판)를 바꿔 볼 수 있다.
...
- 요소는 `id`가 아니라 `data-*` 속성으로 찾는다. 슬라이드 목록 미리보기에서는 `id`가 지워진다.
...
- 확인할 것: 슬라이드가 모두 목록에 뜨는가, 혼자 보기·발표자 모드에서 페이지가 세로로 스크롤되지 않는가, 넘치는 슬라이드가 없는가, 라이트·다크 둘 다 읽히는가, 폭 400px에서 셸이 깨지지 않는가, 콘솔 오류가 없는가.
