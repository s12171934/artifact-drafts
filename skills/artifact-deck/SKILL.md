...
3. `<template id="slides">` 안의 `<slide>`를 내용으로 채운다. 문법은 템플릿 머리 주석을 따른다.
   ...
   - 슬라이드마다 `<notes>`에 발표자가 할 말을 쓴다. 발표 노트의 기본값이 된다. 마크다운(제목·목록·굵게·코드)으로 쓸 수 있고, 발표자 패널에서 서식으로 보이며 쓰는 도중에도 바로 서식으로 바뀐다. 발표자가 고친 노트는 입력을 3초 멈추면 자기 개인 영역에 자동 저장된다. 태그에 맞춘 들여쓰기는 틀이 걷어낸다.
