...
이 저장소의 HTML은 **그대로 Artifact로 퍼블리시될 수 있어야 한다.** 퍼블리시할 때 손대야 하는 초안은 실패한 초안이다.

## 1. 쓰기 전에

1. **`artifact-design` 스킬을 먼저 로드한다.** 페이지 규칙과 디자인 기준의 정본은 그 스킬이다. 아래 체크리스트와 다르면 스킬을 따른다.
2. 초안은 `drafts/<짧은-이름>.html`에 둔다. 함께 올릴 JS·CSS·이미지·데이터는 `drafts/` 안에 두고 상대 경로로 참조한다 (퍼블리시 때 `files`로 함께 올린다).
...
- **외부 리소스 (CSP)**:
  - 스크립트는 `cdnjs.cloudflare.com`(우선), `cdn.jsdelivr.net/npm/`, `cdn.tailwindcss.com`, `code.jquery.com`에서만 가져온다. UMD 빌드를 쓰고, 버전을 정확히 고정하고, 사용하는 인라인 스크립트보다 먼저 둔다.
  - 스타일시트는 Google Fonts만 허용된다. 폰트에는 대체 폰트 스택을 지정한다.
  - 그 외 CSS·JS·이미지는 인라인하거나 `data:` URI 또는 `drafts/` 안의 파일로 둔다.
- **반응형**:
  - 400px 폭에서도 동작해야 한다.
...
**등록은 수정 없이 한다.** 퍼블리시되는 내용은 `drafts/`의 초안 파일과 바이트 단위로 같아야 한다.

- 사용자가 요청하면 초안 파일 경로를 그대로 Artifact 도구의 `file_path`에 넘긴다. 복사본을 만들지 않고, 변환·후처리하지 않고, 스켈레톤 태그도 추가하지 않는다.
- 등록 직전에 규칙 위반을 발견해도 조용히 고쳐서 올리지 않는다. 문제를 사용자에게 알리고 등록을 멈춘다. 사용자가 승인하면 `drafts/`의 초안을 고친 뒤 그 파일을 그대로 올린다.
- 함께 올릴 에셋은 `files`로 넘긴다. 이때도 `drafts/` 안의 원본 파일을 그대로 쓴다.
- 첫 퍼블리시에는 `icon`(일반 명사 한 단어)과 한 문장 `description`을 넣는다. 이 둘은 도구 파라미터라서 HTML을 건드리지 않는다.
- 다시 퍼블리시할 때도 같은 파일 경로를 넘긴다. 그래야 같은 URL로 갱신된다.
